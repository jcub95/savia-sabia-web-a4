...
export function Cart({ onBack, onContinueShopping, onGoHome }: CartProps) {
  ...
  const shipping = subtotal > 0 ? 4000 : 0 // Q40 shipping
  ...
  const handleWhatsAppCheckout = () => {
    const phone = '50238149773'

    const lines = items.map(item => {
      const name = item.blend.displayName[language]
      const variant = item.variant.label[language]
      const qty = item.quantity
      const lineTotal = formatPrice(item.variant.price * qty)
      return `• ${name} — ${variant} x${qty} = ${lineTotal}`
    })

    const subtotalFormatted = formatPrice(subtotal)
    const shippingFormatted = isFreeShipping
    ...
    const mensaje = language === 'es'
      ...
      : [
          ...
          `*Total: ${totalFormatted}*`,
          '',
          'How do I arrange payment and delivery?',
        ].join('\n')
