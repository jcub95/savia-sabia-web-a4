...
export function EnviosContent() {
  ...
  return (
    <main className="min-h-[calc(100vh-4rem)] bg-background px-6 py-16 md:py-24">
      ...
      >
        <h1 className="font-serif font-bold text-foreground text-4xl md:text-5xl mb-12 text-balance">
          {c.header[language]}
        </h1>
        ...
        <section className="mb-14">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <h2 className="font-serif font-semibold text-foreground text-2xl">{c.shippingTitle[language]}</h2>
          </div>
              ...
              <tbody>
                {c.shippingTable.map((row, i) => (
                  <tr key={i} className={i > 0 ? 'border-t border-border' : ''}>
                    <td className="py-3 px-4 text-foreground font-medium">{row.destination[language]}</td>
        ...
        <section className="mb-14">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <h2 className="font-serif font-semibold text-foreground text-2xl">{c.paymentsTitle[language]}</h2>
          </div>
          <div className="space-y-5">
            {c.payments.map((p) => (
              <div key={p.title[language]}>
                <h3 className="text-foreground font-semibold mb-1">{p.title[language]}</h3>
        ...
        <section className="mb-14">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <h2 className="font-serif font-semibold text-foreground text-2xl">{c.orderStepsTitle[language]}</h2>
          </div>
          <ol className="space-y-3">
            {c.orderSteps.map((step, i) => (
              <li key={i} className="flex gap-3 text-sm text-muted-foreground leading-relaxed">
                <span className="shrink-0 size-5 rounded-full bg-primary text-primary-foreground text-xs flex items-center justify-center font-medium">
                  {i + 1}
                </span>
                {step[language]}
              </li>
        ...
        <section className="border-t border-border pt-8">
          <h2 className="font-serif font-semibold text-foreground text-2xl mb-3">{c.returnsTitle[language]}</h2>
